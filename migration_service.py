...
class MigrationService:
    ...
    def migrate_issues_by_keys(self, issue_keys: List[str]) -> List[MigrationResult]:
        ...
        return self._migrate_issues(jira_issues)
    
    def migrate_issues_by_jql(self, jql: str, max_results: int = 50) -> List[MigrationResult]:
        """Migrate Jira issues found by JQL query"""
        rprint(f"[bold blue]Searching for issues with JQL: {jql}[/bold blue]")
        
        # Search for issues
        jira_issues = self.jira_client.search_issues(jql, max_results)
    ...
    def _migrate_issues(self, jira_issues: List[JiraIssue]) -> List[MigrationResult]:
    ...
    def _show_migration_summary(self, results: List[MigrationResult]):
